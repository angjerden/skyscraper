...
// This approach seemed not to work because it misses setting of scriptvariables
// like CUR_ID which engine() handles, therefore we run into null pointers
...
void Logic::talk() {
	...
	// Taking my chances with this one
	logicScript();
	return;
}
...
uint16 Logic::script(uint16 scriptNo, uint16 offset) {
	return scrapeScript(scriptNo, offset);
}
...
bool Logic::fnGetTo(uint32 targetPlaceId, uint32 mode, uint32 c) {
	...
	if (!cpt) {
		...
		std::cout << "can't find _compact's getToTable. Place compact is NULL" << std::endl;
		return false;
	}
	uint16 *getToTable = (uint16 *)_skyCompact->fetchCpt(cpt->getToTableId);
	if (!getToTable) {
		// warning("Place compact's getToTable is NULL");
		std::cout << "Place compact's getToTable is NULL" << std::endl;
		return false;
	}
...
bool Logic::fnChooser(uint32 a, uint32 b, uint32 c) {
	...
	while (*p) {
		...
		char* textBuffer = _skyText->getText(textNum);
...
bool Logic::fnNewSwingSeq(uint32 a, uint32 b, uint32 c) {
	...
	if ((a == 85) || (a == 106) || (a == 75) || (a == 15)) {
		...
		std::cout << "Logic::fnNewSwingSeq: ignored seq " << a << std::endl;
	}
	return true;
}
...
bool Logic::fnPrintf(uint32 a, uint32 b, uint32 c) {
	...
	std::cout << "fnPrintf(" << a << ", " << b << ", " << c << ")" << std::endl;
	return true;
}
