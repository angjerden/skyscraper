...
void writeBMP(const char* filename, uint16 screenFileNr, uint16 paletteFileNr) {
	uint8* screen = _skyDisk->loadFile(screenFileNr);
	uint8* palette = _skyDisk->loadFile(paletteFileNr);
...
// 		free(tmpPal);
// 	} else
// 		std::cout << "Screen::setPalette: can't load file nr. " << fileNum << std::endl;
// }
					...
					if ((spriteComp->status & 4) && // is it sortable playfield?(!?!)
						(spriteComp->screen == Logic::_scriptVariables[SCREEN])) { // on current screen
							DataFileHeader *spriteData =
								(DataFileHeader *)_skyDisk->fetchItem(spriteComp->frame >> 6);
							if (!spriteData) {
								std::cout << "Missing file " << (spriteComp->frame >> 6) << std::endl;
								spriteComp->status = 0;
							} else {
		...
		while (drawList[0]) {
			...
			while ((drawList[0] != 0) && (drawList[0] != 0xFFFF)) {
				...
				if ((spriteData->status & (1 << layer)) &&
					...
					uint8 *toBeDrawn = (uint8 *)_skyDisk->fetchItem(spriteData->frame >> 6);
					if (!toBeDrawn) {
						std::cout << "Spritedata " << (spriteData->frame >> 6) << " not loaded" << std::endl;
						spriteData->status = 0;
					} else {
...
void Screen::drawSprite(uint8 *spriteInfo, Compact *sprCompact) {
	if (spriteInfo == NULL) {
		std::cout << "Screen::drawSprite Can't draw sprite. Data " << (sprCompact->frame >> 6) << " was not loaded" << std::endl;
		sprCompact->status = 0;
		return;
	}
	...
	if ((_sprX + _sprWidth > 320) || (_sprY + _sprHeight > 192)) {
		std::cout << "Screen::drawSprite fatal error: got x = " << _sprX << ", y = " << _sprY << ", w = " << _sprWidth << ", h = " << _sprHeight << std::endl;
		_sprWidth = 0;
		return;
	}
	...
	for (uint32 cntx = 0; cntx < _sprHeight; cntx++) { // start_x | block_loop
		if (grid[gridOfs]) {
			if (!(FROM_LE_16(grid[gridOfs]) & 0x8000)) {
				...
				uint8 *dataSrc = (uint8 *)_skyDisk->fetchItem(Logic::_scriptVariables[layerId]) + gridVal;
	...
	for (uint32 layerCnt = LAYER_1_ID; layerCnt <= LAYER_3_ID; layerCnt++) {
		...
		for (uint32 widCnt = 0; widCnt < _sprWidth; widCnt++) { // x_loop
			...
			while (Logic::_scriptVariables[nLayerCnt + 3]) {
				uint16 *scrGrid;
				scrGrid = (uint16 *)_skyDisk->fetchItem(Logic::_scriptVariables[layerCnt + 3]);
				if (scrGrid[gridOfs]) {
